...
public class PlayerSpawner : ObjectSpawner {
    ...
    private PlayerSpawnData spawnData = new PlayerSpawnData(); // Data container
    ...
    private void generatePlayerSpawnPositions() {
        ...
        spawnData.location.Add(new Tuple<float, float>(topSpawnPosition.x, topSpawnPosition.y));

        int playerType = UnityEngine.Random.Range(0, characterType.Count);
        spawnData.type.Add(playerType);
    }
    ...
    IEnumerator spawnPlayers() {
        ...
        for (int i = 0; i < totalPlayerCount; i++) {
            ...
            GameObject player = characterType[spawnData.type[i]];
            Tuple<float, float> position = spawnData.location[i];
